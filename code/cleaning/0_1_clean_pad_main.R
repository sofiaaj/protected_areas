...
fast_st_difference = function(full_df,st,acres=TRUE){
  ...
  get_difference = function(i,acres=TRUE) {
    print(i)
    print(acres)
    ...
    curr_diff = try(diff_function(curr,F,10^-2))
    ...
    write(
      ...
      file = paste0('code/code_updates/updates.txt'),
      append = TRUE
    )
    return(curr_diff)
  }
...
get_intersections = function(i,check,local,federal) {
  ...
  intersect = intersect %>%
    mutate(
      ...
      year_est = ifelse(year_est>=year_est.1,year_est,year_est.1)
    ) %>%
...
main = function(statefips,type){
  ...
  full_df = bind_rows(pad %>% 
    ...
    st_transform(5070) %>%
    st_set_precision(geo.precision) %>%
    mutate(year_est = ifelse(is.na(year_est) | year_est == -99, 
                             0, 
                             year_est))
  ...
  if(type == 'num_agreements'){
    ...
    num_agreements = merge(num_agreements,
                           bind_rows(num_agreements_lp,num_agreements_sp),
                           by='unique_id') %>%
      mutate(unique_share = new_area/original_area) %>%
      filter(as.numeric(unique_share) >= 0.5) %>%
      filter(as.numeric(new_area) > 100)
    
    filename = paste0('data/temporary_processing_files/num_agreements/',
                      state_name,
                      '.rds')
    ...
    filename = paste0('data/temporary_processing_files/acres/',
                      state_name,
                      '.rds')
